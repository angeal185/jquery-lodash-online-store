...
itemListTpl = _.template('<div class="col s12 m3"><div class="card"><div class="card-image waves-effect waves-block waves-light"><img class="activator" src="{{imgSub}}"></div><div class="card-content"><span class="card-title activator grey-text text-darken-4">{{title}}<i class="material-icons right">more_vert</i></span><p><a href="/{{domain}}item/{{title}}">Show</a></p></div><div class="card-reveal"><span class="card-title grey-text text-darken-4">{{title}}<i class="material-icons right">close</i></span><p>{{description}}</p></div><div class="card-action"><a href="/{{domain}}item/{{title}}">View item</a></div></div></div>'),
...
blogItemsTpl = _.template('<div class="blog-list col s12"><div class="row"><div class="col s4"><img class="blog-img" src="{{img}}"></div><div class="col s8"><h3>{{title}} <span class="blog-date">{{date}}</span></h3><h5>{{sub}}</h5><p class="truncate">{{content}}</p><div class="blog-btm"><p>author: {{author}}</p><a type="button" href="/{{domain}}blog/{{title}}" class="btn waves-effect waves-light blog-btn">Read more</a></div></div></div><hr></div>'),
blogItemTpl = _.template('<div class="container"><h3>{{title}} <span class="blog-date">{{date}}</span></h3><img class="blog-img" src="{{img}}"><h5>{{sub}}</h5><p class="truncate">{{content}}</p><p>author: {{author}}</p><a type="button" href="/{{domain}}blog/" class="btn waves-effect waves-light blog-btn">back</a></div>'),
cartListTpl = _.template('<table><thead><tr id="listObj"></tr></thead><tbody id="listItems"></tbody></table>'),
cartItemTpl = _.template('<tr><td>{{title}}</td><td>{{id}}</td><td><a href="/{{domain}}category/{{category}}">{{category}}</a></td><td>{{price}}</td><td>{{stat}}</td><td><a href="/{{domain}}item/{{title}}">view</a></td><td><a target="_blank" href="{{link}}">Buy</a></td><td><button type="button" id="{{id}}" class="btn cartDel">Remove</button></td></tr>'),
